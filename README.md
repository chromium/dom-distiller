...
-   Reader Mode not being offered on pages where it should be.
...
version of Chrome. You may need to enable the feature via accessibility settings
in order for the infobar to enter Reader Mode can appear for a page.

You may also configure the triggering logic by following these steps:
...
-   Run `ant package` and copy the generated files into Chrome. You can use this
...
You can distill web pages by selecting the menu item `Toggle distilled page
contents` or activating the Reader Mode icon when it appears in the omnibox.
...
1.  Select to load an unpacked extension and point to the `out/extension`
    folder.
...
Chrome, whereas the rest of the steps are typically done in your own build of
