...
1.  Navigate to [chrome://flags](chrome://flags) and search for "Reader mode"
...
1.  Choose "Non-mobile-friendly article" to turn on Reader mode for
...
1.  Next time you're trying to read a page, tap on the "Make page
...
1.  Install JDK 7 using either your organizations software management tool, or
    download it from
...
1.  Since both the protocol buffer compiler and Python bindings are needed,
...
1.  Download ChromeDriver (chromedriver_mac32.zip) from the
...
1.  Install the PyPI package management tool `pip` by running:
...
1.  Run `ant package` and copy the generated files into Chrome. You can use this
...
1.  From `$CHROME_SRC` run GN to setup ninja build files using
...
1.  Run the `components_browsertests` binary to execute the tests:
...
-   Select which tests to run using the `--gtest_filter` flag:
