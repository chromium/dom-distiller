...
1.  Select one of the following options from the dropdown menu to change on what
    types of pages Chrome offers Reader Mode:
    -   **Non-mobile-friendly article**: Articles and article-like pages that
        render poorly on mobile devices. This is the default behavior.
    -   **All articles**: All articles and article-like pages.
    -   **Always**: All pages regardless of content.
1.  Click "Relaunch Now" at the bottom of the page.
1.  Navigate to an elligible page, as determined by the selected triggering
    algorithm.
1.  Tap the "Show simplified view" infobar at the bottom of the page.
...
-   `ant test -Dtest.filter=$FILTER_PATTERN`.
